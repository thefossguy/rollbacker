...

{
  ...
  config =
    let
      groupedFileSystems = builtins.groupBy (mountPoint: mountPoint.device) (
      ...
      systemdUnits = builtins.map (
        ...
        let
          ...
          deviceMountPoints = builtins.map (
            localMountPointConfigSet: localMountPointConfigSet.mountPoint
          ) mountPointConfigSet;
          systemdDeviceMountPoints = (
            builtins.map (
              deviceMountPoint: "${utils.escapeSystemdPath deviceMountPoint}.mount"
            ) deviceMountPoints
          );
